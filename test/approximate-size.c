...
static void
on_approximate_size(rocksdb_approximate_size_t *req, int status) {
  int e;
  ...
  rocksdb_approximate_size_cleanup(req);
}
...
on_write(rocksdb_write_batch_t *req, int status) {
  ...
  static rocksdb_approximate_size_t approximate_size;
...
main() {
  ...
  e = rocksdb_open(loop, &db, &open, "test/fixtures/approximate-size.db", &options, &descriptor, &family, 1, NULL, on_open);
  assert(e == 0);
